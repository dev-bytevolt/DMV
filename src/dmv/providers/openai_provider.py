...
class OpenAIExtractionProvider(ExtractionProvider):
    ...
    async def extract_pdf(
        ...
        *,
        ...
        logger.info("Extracting %s (%s)", pdf_path.name, document_type)
        file_id: str | None = None
        try:
            file_id = await self._upload_pdf(pdf_path)
            return await self._extract_uploaded_file(
                file_id,
                pdf_path.name,
    ...
    async def _extract_uploaded_file(
        self,
        file_id: str,
        pdf_name: str,
        *,
        ...
        logger.info(
            "OpenAI POST /v1/responses — extract %s (model=%s, file_id=%s)",
            pdf_name,
            self._settings.openai_model,
            file_id,
        )
        ...
        payload = parse_json_response(response.output_text)
        logger.info(
            "OpenAI POST /v1/responses — extracted %s; tokens: %s input, %s output, %s total",
            pdf_name,
            response.usage.input_tokens,
            response.usage.output_tokens,
            response.usage.total_tokens,
        )
        return ExtractionOutcome(result=payload, usage=response.usage)
